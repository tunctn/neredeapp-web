...
const tvMediaCache = new DataCache(api.tvMedia);
const tvProvidersCache = new DataCache(api.providers);
...
export const getStaticProps: GetStaticProps = async ({ params: { id } }) => {
  ...
  const tvMediaResponse = await tvMediaCache.getData(id);
  const tv = transformSingleTv(tvResponse, tvMediaResponse);
  const providersResponse = await tvProvidersCache.getData({
    id: id,
    type: "tv",
  });
  const providers = transformProviders(providersResponse);

  return { props: { tv, providers }, revalidate: true };
};
          ...
          >
            ...
            <div>
              ...
              <div className="mt-8 space-y-4">
                <h2 className="text-lg font-bold ">Nereden izlenir?</h2>
                <ul>
                  {providers.providers.map((provider, index) => {
                    console.log(provider);
                    return (
                      <li className="space-y-2" key={provider.host + index}>
                        <a
                          href={provider.url}
                          target="_blank"
